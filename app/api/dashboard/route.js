...
export async function GET(req) {
  try {
    ...
    // Inactivity Nudge
    if (user.hasSaved === false && user.hasSentInactivityMail === false) {
      const sixHours = 6 * 60 * 60 * 1000;
      const timeSinceCreation = Date.now() - new Date(user.createdAt).getTime();
      
      if (timeSinceCreation >= sixHours) {
        user.hasSentInactivityMail = true;
        await user.save();
        ...
        const emailHtml = `
          <div style="font-family: sans-serif; color: #111;">
            ...
            <p>Hi ${user.name},</p>
            <p>You haven’t started saving yet. Even a small amount today can help build a habit.</p>
          </div>
        `;
        sendEmail({
          to: user.email,
          subject: "Start your savings journey",
          html: emailHtml
        });
      }
    }
